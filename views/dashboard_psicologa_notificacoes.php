<!--
    ARQUIVO: views/dashboard_psicologa_notificacoes.php
    DESCRIÇÃO: View da aba "Notificações" do dashboard da psicóloga.

    Este arquivo é incluído pelo dashboard_psicologa.php quando a aba 'notificacoes'
    está ativa. Diferente das outras views, esta NÃO usa PHP para renderizar as
    notificações — ela usa JavaScript com Fetch API para carregar os dados de
    forma assíncrona (AJAX) a partir do endpoint api/notificacoes.php.

    FLUXO:
      1. A página carrega com um estado de "Carregando..."
      2. O DOMContentLoaded dispara notif.init()
...
<!-- ═══════════════════════════════════════════════════════════
     ESTRUTURA HTML DA ABA DE NOTIFICAÇÕES
═══════════════════════════════════════════════════════════ -->
<div class="secao">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 12px;">
        ...
        <div class="notificacoes-header-acoes" style="display: flex; gap: 8px;">
            ...
            <!-- O parâmetro 'true' em recarregar(true) atualiza o badge de contagem -->
...
/*
 * OBJETO notif
 * Gerencia todas as operações de notificações desta view.
...
var notif = {
    ...
    renderizar: function(lista, naoLidas) {
        ...
        for (var i = 0; i < lista.length; i++) {
            ...
            // esc() escapa o texto para prevenir XSS (injeção de HTML)
    ...
    marcarLida: function(id, btn) {
        var self = this;
        // FormData: formato de dados para envio via POST (equivalente a um formulário)
        var formData = new FormData();
        formData.append('acao', 'marcar_lida');
        formData.append('id_notificacao', id);

        // Envia requisição POST para o endpoint de notificações
